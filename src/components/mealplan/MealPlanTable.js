      ...
      {mealPlans.map((plan) => (
        ...
        >
          ...
          <div className="flex-1 flex flex-col">
            <h2 className="text-lg font-bold text-gray-800 mb-2">{plan.plan_name}</h2>
            <p className="text-gray-600 mb-4 flex-1 line-clamp-3">{plan.description || 'ไม่มีคำอธิบาย'}</p>
            ...
            <div className="grid grid-cols-1 gap-2">
              <Link href={`/mealplan-Add?plan_id=${plan.plan_id}`}>
                <button className="w-full px-4 py-2 bg-green-100 text-green-600 rounded-lg hover:bg-green-200">
                  จัดการแผนอาหาร
                </button>
              </Link>
